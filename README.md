...
- **Autosplitter**: drives LiveSplit through its built-in TCP server (right-click LiveSplit, Control, Start TCP Server; port 16834). The console reports run events (start, split candidates with a reason, reset, and for Deadlocked a pause and resume around a quit to the XMB); this panel decides what to do with them: which reasons split, a planet route that only splits when the entered planet matches the upcoming split's name (lists in `data/autosplit/`), and never-reset for categories that need it. No timer of its own: LiveSplit is the clock.
